	...
	if (p->tree_com == NULL) {
		...
		if (Is_Stable(p->tree_com, node_type, inner_flag, lf_below) == 1) {
			...
			if (unstb_ret >= 0) {
				...
				printf("  Finish processing unstb_rets\n");
				...
				*no_break = *no_break + 1;
				...
				// The ret node below is not Inner, but revised
				x = p->ret_node;
...
void main(int argc, char *argv[]) {
	...
	printf("leaf_below: post-order:\n");
	for (i=0; i<n_r; i++) printf("%s: %d, %d) ", node_strings[r_nodes[i]],
	lf_below[r_nodes[i]], Inner_flag[r_nodes[i]]);
	printf("\n");
