	...
	if (p->tree_com == NULL) {
		...
		if (no_rets_in > 0 && no_rets_out == 0){
			for (i = 0; i < no_rets_in; i++) {
				...
				Modify(p, p, node_type, unstb_ret);
			}
		...
		else if (no_rets_out > 0 && no_rets_in == 0){
			// remove this component from further consideration in future
			for (i = 0; i < no_rets_out; i++) {
				unstb_ret = unstb_rets_out[i];
				if (inner_flag[unstb_ret] == CROSS) {
					inner_flag[unstb_ret] = INNER;
					super_deg[unstb_ret] = 1;
				}
				Modify(p, p, node_type, unstb_ret);
			}
