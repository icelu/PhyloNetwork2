...
int Cluster_Containment(struct components *ptr, int r_nodes[], int n_r,
		...
		int super_deg[], struct components *cps, struct lnode *child_array[], struct lnode *parent_array[],
  ...
  printf("\n");
	...
	if (p->tree_com == NULL) {
		...
		return Cluster_Containment(p->next, r_nodes, n_r, no_nodes, node_type,
				...
				in_cluster, super_deg, cps, child_array, parent_array, n_l, no_break);
	}
			...
			if (no_slf == 1){
				if (sleaves[0] == input_leaves[0]){
					...
					return Cluster_Containment(p->next, r_nodes, n_r, no_nodes,
							...
							input_leaves, in_cluster, super_deg, cps, child_array, parent_array, n_l,
							no_break);
				}
			}
			else
			...
			if (is_cluster >= 0) {
				...
				if (count_out == no_slf) {
					...
					res = Cluster_Containment(p->next, r_nodes, n_r, no_nodes,
							...
							child_array, parent_array, n_l, no_break);
		...
		// split into two networks at first
			...
			*no_break = *no_break + 1;
			...
			for (i = 0; i < no_rets_out; i++) {
				unstb_ret = unstb_rets_out[i];
				int curr_leaf = lf_below[unstb_ret]; 	// not in B
				// find its parents in p1
				struct lnode* parent = parent_array[unstb_ret];
				while(parent!=NULL && run_1st == 1){
					struct lnode* child = child_array[parent->leaf];
					while(child!=NULL)
					{
						int a_leaf = child->leaf;
						if (node_type[a_leaf]==RET)
						{
							a_leaf = lf_below[a_leaf];
						}
						if (Is_In(a_leaf, input_leaves, no1) == 1 && a_leaf!=curr_leaf){
							run_1st = 0;
							break;
						}
						child = child->next;
			...
			for (i = 0; i < no_rets_in; i++) {
				unstb_ret = unstb_rets_in[i];
				int curr_leaf = lf_below1[unstb_ret]; 	// in B
				// find its parents in p1
				struct lnode* parent = parent_array[unstb_ret];
				while(parent!=NULL && run_2nd == 1){
					struct lnode* child = child_array[parent->leaf];
					while(child!=NULL)
					{
						int a_leaf = child->leaf;
			...
			if (res == 0 && run_2nd == 1) {
				...
				res = Cluster_Containment(p1->next, r_nodes, n_r, no_nodes, node_type,
					...
					whole_copy, child_array, parent_array, n_l, no_break);
			}
			return res;
		}
		else {
...
void main(int argc, char *argv[]) {
	...
	res = Cluster_Containment(p, r_nodes, n_r, no_nodes, node_type, inner_flag,
			...
			all_cps, child_array, parent_array, n_l, &no_break);
